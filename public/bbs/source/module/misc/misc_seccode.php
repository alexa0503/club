...
if($_GET['action'] == 'update') {
	...
	if($_G['setting']['seccodedata']['type'] == 2) {
      ...
      // $message = lang('core', 'seccode_image'.$ani.'_tips').'<img onclick="updateseccode(\''.$idhash.'\')" width="'.$_G['setting']['seccodedata']['width'].'" height="'.$_G['setting']['seccodedata']['height'].'" src="misc.php?mod=seccode&update='.$rand.'&idhash='.$idhash.'" class="vm" alt="" />';
			$message = '<img onclick="updateseccode(\''.$idhash.'\')" width="'.$_G['setting']['seccodedata']['width'].'" height="'.$_G['setting']['seccodedata']['height'].'" src="misc.php?mod=seccode&update='.$rand.'&idhash='.$idhash.'" class="vm yzcode" alt="" />';
		}
	}
...
if($('$showid')) {
	if(!$('v$showid')) {
      ...
      '<span id="v$showid">$message</span>' + sectplcode[3] +
			'<span id="checkseccodeverify_$idhash" class="tipsIconWrap"><img src="' + STATICURL + 'image/common/none.gif" width="16" height="16" class="vm" /></span>';
		evalscript(string);
